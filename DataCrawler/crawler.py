...
def _google_search(keyword: str) -> List[str]:
    ...
    Uses keyword to google search and returns a list of link where each element is link to individual website in
    the top results.
    """
    base_url = "https://www.google.com/search?q="
    result_url = base_url + quote_plus(keyword)     # concatenate to create a full url to the search result page

    option = webdriver.ChromeOptions()
    option.add_argument("headless")     # An option that prevents a chrome browser from opening
...
def _get_contents(url: str) -> List[str]:
    ...
    :return:
    A list of strings where its elements are string lines of main contents of the website.
    ...
    contents_html = chrome_driver.page_source       # convert html file into string

    soup = BeautifulSoup(contents_html, "html.parser")

    contents = soup.select("p")
    # "p" is used to distinguish contents surrounded by <p> tags from useless information
    # must be fixed since it cannot recognize strings surrounded by <a> tags and etc.
...
def crawl(keyword: str) -> List[List[str]]:
    ...
    A keyword or sentence(s) to look up on Google.
    :return:
    A list of strings where its elements are string lines of main contents from each website in the top results.
    """
